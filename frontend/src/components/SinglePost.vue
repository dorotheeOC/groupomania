...
<section id="main">
    <div class="mx-4">
        ...
        <div class="col-lg-6 p-4 my-4 mx-auto">
            <div class="card py-4" >
                <span @click="goBack" class="back font-weight-bold px-4">	&larr; Retour</span>
            <div class="post-title pl-4 my-0">
                ...
                <div class="d-md-flex justify-content-start align-items-center">
                    <div class="card-text">
                        <small class="text-muted">De <span class="link" @click="getProfile(post.userId)">{{post.author}} </span></small>
                        <small class="text-muted mr-2" v-if="post.createdAt"> le {{dateFormat(post.createdAt)}}</small>
                    </div>
                    <span class="edit d-md-block" @click="editOnePost" v-if="post.userId === currentUser.id"><i class="fas fa-pen mr-1 p-0"></i>Modifier</span>
                </div>
                
            </div>
            <div class="card-body">
                <div class="card-text" v-html="post.postContent"></div>
                <div class="d-md-flex justify-content-start align-items-center stat-single">
                    ...
                    <span class="report text-muted d-block ml-md-3 p-1" 
            ...
            <ul class="list-group list-group-flush pt-4 p-md-4">
                ...
                <li class="list-group-item ml-2"  v-for="(comment, index) in post.postComment" :key="comment.index">
                    <div class="card-text my-0 ml-lg-2 p-0">
                        <small class="text-muted" v-if="comment.userId !== null">De <span @click="getProfile(comment.userId)">{{comment.author}} </span></small>
                    ...
                    <div class="d-lg-flex align-items-center">
                        ...
                        <small>
                            <span class="report text-muted ml-3" 
                            @click.prevent="deleteOneComment(comment.id)" 
                        ...
                        <small>
                            ...
                            v-if="currentUser.role === 'user' && comment.userId !== currentUser.id">
